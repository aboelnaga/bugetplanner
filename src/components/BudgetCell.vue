<template>
    <div class="text-center relative" :class="getCellTextColorClass(data)">
        <div v-if="cellData" class="font-medium cursor-help" :title="cellData.tooltip || ''">

            <!-- Both Mode Display -->
            <template v-if="cellData.template === 'both'">
                <div class="actual-expected-display">
                    <div class="actual">{{ cellData.actual || '—' }}</div>
                    <div class="expected">{{ cellData.expected || '—' }}</div>
                </div>
            </template>

            <!-- Single Mode Display -->
            <template v-else>
                <div class="single-mode-display" :class="cellData.classes || ''">
                    {{ cellData.value || '—' }}
                    <span v-if="cellData.closed" class="text-xs ml-1 text-green-600 dark:text-green-400 cursor-help"
