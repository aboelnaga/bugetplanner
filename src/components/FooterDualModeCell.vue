<template>
    <div :class="getFooterCellClasses(getRawValueForCSS(data), itemType)" class="whitespace-nowrap">
        <template v-if="getFooterDualData(data, itemType)?.type === 'both'">
            <div v-if="getFooterDualData(data, itemType)?.closed" class="footer-dual-mode">
                <div class="actual">{{ getFooterDualData(data, itemType)?.actual }}</div>
                <div class="expected">{{ getFooterDualData(data, itemType)?.expected }}</div>
                <span class="text-xs ml-1 text-green-600 dark:text-green-400 cursor-help"
                    :title="closedTooltip">●</span>
            </div>
        ...
        <template v-else>
            ...
            <div v-else>{{ getFooterDualData(data, itemType)?.value }}</div>
        </template>
    </div>
</template>
...
const getFooterCellClasses = (amount, itemType) => {
    ...
    switch (itemType) {
        ...
        case 'net':
            return `${baseClasses} ${amount > 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`
        default:
            return baseClasses
    }
}
