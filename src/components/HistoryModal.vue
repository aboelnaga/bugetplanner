  ...
  <div v-if="modelValue" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
      ...
      <div class="flex-1 overflow-y-auto p-6">
        <!-- History Content -->
        <div v-if="historyItems.length > 0" class="space-y-6">
          <!-- Summary Stats -->
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="bg-gradient-to-r from-orange-50 to-amber-50 p-4 rounded-lg border border-orange-200">
              <div class="flex items-center justify-between">
                <div>
                  <p class="text-sm font-medium text-orange-600">Total Impact</p>
                  <p class="text-2xl font-bold text-orange-700">{{ formatCompactCurrency(calculateTotalImpact()) }}</p>
                </div>
          ...
          <div class="space-y-6">
            <div v-for="(itemChanges, budgetItemId) in groupedHistoryItems" :key="budgetItemId" 
              ...
              <div class="bg-gray-50 px-4 py-3 border-b border-gray-200">
                <div class="flex items-center justify-between">
                  <div class="flex items-center space-x-3">
                    ...
                    <h4 class="font-semibold text-gray-900">{{ getBudgetItemName(budgetItemId) }}</h4>
      ...
      <div class="flex justify-between items-center p-6 border-t border-gray-200 bg-gray-50">
        ...
        <button @click="closeModal" class="px-6 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
          Close
        </button>
      </div>
    </div>
